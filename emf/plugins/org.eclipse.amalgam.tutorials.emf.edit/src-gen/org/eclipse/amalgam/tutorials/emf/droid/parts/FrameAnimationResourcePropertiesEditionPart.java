/**
 * Generated with Acceleo
 */
package org.eclipse.amalgam.tutorials.emf.droid.parts;

// Start of user code for imports
import org.eclipse.emf.ecore.EObject;
import org.eclipse.emf.eef.runtime.ui.widgets.referencestable.ReferencesTableSettings;
import org.eclipse.jface.viewers.ViewerFilter;



// End of user code

/**
 * 
 * 
 */
public interface FrameAnimationResourcePropertiesEditionPart {

	/**
	 * @return the name
	 * 
	 */
	public String getName();

	/**
	 * Defines a new name
	 * @param newValue the new name to set
	 * 
	 */
	public void setName(String newValue);


	/**
	 * @return the oneShot
	 * 
	 */
	public Boolean getOneShot();

	/**
	 * Defines a new oneShot
	 * @param newValue the new oneShot to set
	 * 
	 */
	public void setOneShot(Boolean newValue);




	/**
	 * Init the frames
	 * @param current the current value
	 * @param containgFeature the feature where to navigate if necessary
	 * @param feature the feature to manage
	 */
	public void initFrames(ReferencesTableSettings settings);

	/**
	 * Update the frames
	 * @param newValue the frames to update
	 * 
	 */
	public void updateFrames();

	/**
	 * Adds the given filter to the frames edition editor.
	 * 
	 * @param filter
	 *            a viewer filter
	 * @see org.eclipse.jface.viewers.StructuredViewer#addFilter(ViewerFilter)
	 * 
	 */
	public void addFilterToFrames(ViewerFilter filter);

	/**
	 * Adds the given filter to the frames edition editor.
	 * 
	 * @param filter
	 *            a viewer filter
	 * @see org.eclipse.jface.viewers.StructuredViewer#addFilter(ViewerFilter)
	 * 
	 */
	public void addBusinessFilterToFrames(ViewerFilter filter);

	/**
	 * @return true if the given element is contained inside the frames table
	 * 
	 */
	public boolean isContainedInFramesTable(EObject element);





	/**
	 * Returns the internationalized title text.
	 * 
	 * @return the internationalized title text.
	 * 
	 */
	public String getTitle();

	// Start of user code for additional methods
	
	// End of user code

}
