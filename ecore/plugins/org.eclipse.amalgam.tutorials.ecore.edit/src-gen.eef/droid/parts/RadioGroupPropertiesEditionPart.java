/**
 * Generated with Acceleo
 */
package droid.parts;

// Start of user code for imports
import org.eclipse.emf.ecore.EObject;
import org.eclipse.emf.eef.runtime.ui.widgets.referencestable.ReferencesTableSettings;
import org.eclipse.jface.viewers.ViewerFilter;



// End of user code

/**
 * 
 * 
 */
public interface RadioGroupPropertiesEditionPart {

	/**
	 * @return the name
	 * 
	 */
	public String getName();

	/**
	 * Defines a new name
	 * @param newValue the new name to set
	 * 
	 */
	public void setName(String newValue);




	/**
	 * Init the radioButtons
	 * @param settings settings for the radioButtons ReferencesTable 
	 */
	public void initRadioButtons(ReferencesTableSettings settings);

	/**
	 * Update the radioButtons
	 * @param newValue the radioButtons to update
	 * 
	 */
	public void updateRadioButtons();

	/**
	 * Adds the given filter to the radioButtons edition editor.
	 * 
	 * @param filter
	 *            a viewer filter
	 * @see org.eclipse.jface.viewers.StructuredViewer#addFilter(ViewerFilter)
	 * 
	 */
	public void addFilterToRadioButtons(ViewerFilter filter);

	/**
	 * Adds the given filter to the radioButtons edition editor.
	 * 
	 * @param filter
	 *            a viewer filter
	 * @see org.eclipse.jface.viewers.StructuredViewer#addFilter(ViewerFilter)
	 * 
	 */
	public void addBusinessFilterToRadioButtons(ViewerFilter filter);

	/**
	 * @return true if the given element is contained inside the radioButtons table
	 * 
	 */
	public boolean isContainedInRadioButtonsTable(EObject element);





	/**
	 * Returns the internationalized title text.
	 * 
	 * @return the internationalized title text.
	 * 
	 */
	public String getTitle();

	// Start of user code for additional methods
	
	// End of user code

}
